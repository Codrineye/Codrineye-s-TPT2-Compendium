...
Fix the previous change so that we can actually make progress by entering another game of towertesting.
Prior to this fix, the incrementation lock would get unlocked because close.towertesting() would trigger and set the lock back to 0 before we could get the impulse game.newround()

### version 2.3.0

Add an incrementation lock that prevents the script to advance the counting string blindly.<br>
It will now only advance it if it actually made progress by entering another game of towertesting.

### version 2.2.9

Make the code no longer exit towertesting if shut down.

### version 2.2.8

Add a third condition comment that documents what version the user has imported.

### version 2.2.7

Update source so the lua no longer needs the linebreaks.<br>
Fix a bug introduced in the last update which made macro `#diff_list(switch)` show some of the coordinates.<br>
Rename the `hider` name to no longer parse the `<size=0>` block.<br>
Rename `Curent Mode` to `Current Mode`.
