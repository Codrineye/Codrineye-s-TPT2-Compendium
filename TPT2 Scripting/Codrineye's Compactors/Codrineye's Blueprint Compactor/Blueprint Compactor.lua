...
function BP.comp.recorded_blueprint(name, loopable)
    ...
    -- Get the amount of space we've currently used up
...
function BP.comp.blueprint_base(defined_base)
    ...
    -- We also have to update the total amount of space being used
...
function BP.comp.sync(spell_amount)
    -- Perform basic error handling on the parameter
    if tonumber(spell_amount) == nil then
        err_msg[1] = "Trying to add a non-number as a sync amount";
    elseif tonumber(spell_amount) <= 0 then
        err_msg[1] = "Trying to add a sync amount that's less than 1";
    elseif math.type(tonumber(spell_amount)) ~= "integer" then
        err_msg[1] = "Trying to assign a non-integer as a sync amount";
    end
    BP.debug.detect_error();
    --[[/*
        ...
        * 
        * We subtract the amount of spells that are synchronized by 1
        * so that the final synchronized spell refreshes the budget
       */]]
    BP.sync_amount = tonumber(spell_amount) - 1;
    -- sync_amount always gets overwritten
end
...
function BP.comp.add_spell(name, timer, coord_x, coord_y)
    ...
    --[[/*
       ...
       */]]
    if BP.sync_amount > 0 then
        new_spell.sync = "s";
        BP.sync_amount = BP.sync_amount - 1;
    end
